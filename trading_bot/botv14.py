...
"""
PHILOSOPHIE DE YUICHI:

1. N'AGIT QUE quand il a 3+ coups d'avance
2. OBSERVE le comportement pour détecter les patterns
3. EXPLOITE la cupidité et la peur des autres
4. RETOURNE les manipulations contre les manipulateurs
5. PATIENCE ABSOLUE - attend le setup PARFAIT
6. FRAPPE FORT quand l'adversaire est piégé dans son propre jeu

APPLICATION AU TRADING:

1. N'entre QUE quand les whales/market makers sont piégés
2. Détecte les fausses breakouts AVANT qu'elles ne reverse
3. Exploite les liquidations en cascade
4. Entre CONTRE le sentiment quand c'est un piège évident
5. Attend des heures/jours pour LE setup parfait
6. Position size agressive quand setup "Game Over"
"""
...
logging.basicConfig(
    ...
    level=logging.INFO,
    handlers=[
        logging.FileHandler("yuichi_bot_v14_true.log"),
...
# ---------------------------------------------------------------------
# YUICHI CONFIG - PATIENCE & PRECISION
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# YUICHI CORE: TRAP DETECTION & GAME STATE
# ---------------------------------------------------------------------
...
def yuichi_entry_signal(multi_tf_data, psychology) -> tuple:
    ...
    logger.info(f"?? YUICHI SIGNAL: {signal_type.upper()} | Score: {score:.1f}/10 | Trap: {trap.value if trap else 'none'}")
...
price_history: List[float] = []  # kept if you want to later add plotting
...
def update_winnings_box(entry_var=None):
    # CLI version: just log current winnings
    logger.info(f"Cumulative winnings: ${config.cumulative_winnings:.2f}")
...
def update_psychology_panel(box, psychology):
    if psychology is None:
        return
    logger.info(
        f"Psychology | Sentiment: {psychology.sentiment.value} | F&G: {psychology.fear_greed_index:.0f} "
...
def close_trade(reason, exit_price, log_box=None, winnings_var=None):
    ...
    log_msg = (
        ...
        f"?? {reason} [{result}] ??\n"
        f"{'='*70}\n"
        f"Setup: {trade_state.game_state.value if trade_state.game_state else 'unknown'}\n"
        ...
        f"Game Over Setups: {performance.game_over_setups} ({performance.game_over_wins} wins)\n"
        f"{'='*70}\n"
    )
...
def execute_yuichi_strategy(log_box=None, ax=None, canvas=None, winnings_var=None, stats_box=None, psych_box=None):
    logger.info("=" * 70)
    logger.info("?? YUICHI TRUE METHOD ACTIVATED (CLI) ??")
        ...
        try:
            ...
            if not trade_state.active:
                ...
                if expected_profit_pct < config.min_profit_threshold:
                    logger.info(f"Expected profit {expected_profit_pct:.2%} < threshold, skip")
                    time.sleep(5)
                    continue
...
if __name__ == "__main__":
    logger.info("?? Initializing YUICHI TRUE METHOD (CLI) ??")
    logger.info(f"Capital: ${config.capital}")
    logger.info("Philosophy: Observe. Detect. Wait. Strike.")
