...
# Forcer l'output console en UTF-8 (sécurité, même si on n'utilise plus d'emoji)
...
class YuichiConfig:
    ...
    # Interprétation : montants notionnels en USDT pour la position
    martingale_steps = [8, 12, 25, 50, 100, 200, 400, 800]
    max_martingale_steps = 6

    rsi_period = 14
    # on facilite les BUY et on rend les SELL "overbought" plus rares
    ...
    # encore plus souple pour avoir des signaux
    min_confirmation_layers = 2
...
def analyze_whale_activity(order_book_data):
    ...
    - ratio clampé pour éviter les 200x
    """
    ...
    min_depth = 5.0  # ex: 5 BTC minimum sur les 10 premiers niveaux
...
def yuichi_entry_signal(multi_tf_data, psychology):
    """
    Logique assouplie :
    - On accumule des confirmations
    - On n'interdit plus les SELL en uptrend H1, on les marque juste
    - Fallback structurel pour être sûr d'avoir des signaux
    """
    ...
    # ---------- 1) Psychologie globale (fear / greed) ----------
    if (
        psychology.sentiment == MarketSentiment.FEAR
        and psychology.fear_greed_index < 50
    ):
        ...
        and psychology.fear_greed_index > 80
    ):
    ...
    # ---------- 4) Position par rapport au VWAP ----------
    ...
    if trend_up_oversight is not None:
        ...
        elif signal_type == "sell" and trend_up_oversight:
            confirmations.append("against_1h_uptrend")

    # ---------- 8) Fallback structurel si aucun camp clair ----------
    if signal_type is None:
        if rsi < 45 and price < vwap and ema9 > ema21:
            signal_type = "buy"
            confirmations.append("structural_buy_bias")
        elif rsi > 55 and price > vwap and ema9 < ema21:
            signal_type = "sell"
            confirmations.append("structural_sell_bias")

    # ---------- 9) Fallback extrême RSI ----------
    if (not signal_type) or (len(confirmations) < config.min_confirmation_layers):
        if rsi < config.rsi_extreme_oversold - 5:
            signal_type = "buy"
            if "forced_buy_rsi_extreme" not in confirmations:
                confirmations.append("forced_buy_rsi_extreme")
        elif rsi > config.rsi_extreme_overbought + 5:
            signal_type = "sell"
            if "forced_sell_rsi_extreme" not in confirmations:
                confirmations.append("forced_sell_rsi_extreme")

    # ---------- 10) Validation finale ----------
    if not signal_type:
        return None, None, "No clear bias"

    if len(confirmations) < config.min_confirmation_layers:
        if not any(c.startswith("forced_") for c in confirmations):
...
def calculate_position_size(psychology, step):
    # step = index martingale, valeur = notional en USDT
...
class TradeState:
    def __init__(self):
        ...
        # pour shrink temporel
        self.original_stop_loss = None
        self.original_take_profit = None

        # ---- PnL réaliste ----
        self.position_notional = 0.0  # taille en USDT
        self.position_qty = 0.0       # quantité BTC
...
def update_chart(ax, canvas):
    ...
    # 2) Lignes horizontales affichées (entry / SL / TP)
    range_levels = []      # ce qu'on utilise pour fixer le Y-range
    current_levels = []    # juste pour info si besoin
    ...
    if trade_state.entry_price is not None:
        ...
        range_levels.append(trade_state.entry_price)
    ...
    if trade_state.stop_loss is not None:
        ...
        current_levels.append(trade_state.stop_loss)
    ...
    if trade_state.take_profit is not None:
        ...
        current_levels.append(trade_state.take_profit)

    # 👉 Ici le trick : on fixe la fenêtre avec les SL/TP d’origine
    if trade_state.original_stop_loss is not None:
        range_levels.append(trade_state.original_stop_loss)
    if trade_state.original_take_profit is not None:
        range_levels.append(trade_state.original_take_profit)

    if not range_levels:
        range_levels = current_levels

    # 3) Calcul du range Y
    y_values = list(price_history) + range_levels
    if y_values:
        y_min = min(y_values)
        y_max = max(y_values)
...
def tighten_sl_tp_over_time(trade_duration_minutes):
    """
    Resserre progressivement la distance SL/TP autour de l'entry
    en fonction du temps passé dans le trade.
    """
    ...
    effective_horizon = max(1.0, config.max_hold_time_minutes * 0.5)
    progress = min(1.0, max(0.0, trade_duration_minutes / effective_horizon))

    min_shrink = 0.2
    shrink = 1.0 - (1.0 - min_shrink) * progress
    ...
    if trade_state.type == "buy":
        ...
        trade_state.stop_loss = min(new_sl, entry - 1e-8)
        trade_state.take_profit = max(new_tp, entry + 1e-8)
        ...
        new_sl = entry + dist_sl0 * shrink
        new_tp = entry - dist_tp0 * shrink
        trade_state.stop_loss = max(new_sl, entry + 1e-8)
        trade_state.take_profit = min(new_tp, entry - 1e-8)
...
def close_trade(reason, profit_loss, exit_price, log_box, stats_box, step_box, winnings_var):
    ...
    trade_data = {
        ...
        "position_notional": trade_state.position_notional,
        "position_qty": trade_state.position_qty,
    }
    ...
    log_msg = (
        ...
        f"Position: ${trade_state.position_notional:.2f} (~{trade_state.position_qty:.6f} BTC)\n"
    ...
    # reset trade state de base
    trade_state.active = False
    trade_state.type = None
    trade_state.position_notional = 0.0
    trade_state.position_qty = 0.0
        ...
        try:
            ...
            if not trade_state.active:
                ...
                if signal_type and confirmations:
                    ...
                    # ---- nouvel état PnL réaliste ----
                    trade_state.position_notional = float(position_size)
                    trade_state.position_qty = (
                        float(position_size) / current_price if current_price > 0 else 0.0
                    )
                    ...
                    log_msg = (
                        ...
                        f"Position: ${position_size:.2f} (~{trade_state.position_qty:.6f} BTC)\n"
                ...
                qty = trade_state.position_qty

                if trade_duration > config.max_hold_time_minutes:
                    if trade_state.type == "buy":
                        profit_loss = (current_price - trade_state.entry_price) * qty
                    else:
                        profit_loss = (trade_state.entry_price - current_price) * qty
                    close_trade(
                        "Time limit",
                        profit_loss,
                        current_price,
                ...
                if trade_state.type == "buy":
                    ...
                    if current_price >= trade_state.take_profit:
                        profit_loss = (trade_state.take_profit - trade_state.entry_price) * qty
                        close_trade(
                            "TAKE PROFIT",
                            profit_loss,
                            trade_state.take_profit,
                        ...
                        profit_loss = (trade_state.trailing_stop - trade_state.entry_price) * qty
                        close_trade(
                        ...
                        profit_loss = (trade_state.stop_loss - trade_state.entry_price) * qty
                        close_trade(
                            "Stop Loss",
                            profit_loss,
                            trade_state.stop_loss,
                    ...
                    if current_price <= trade_state.take_profit:
                        profit_loss = (trade_state.entry_price - trade_state.take_profit) * qty
                        close_trade(
                            "TAKE PROFIT",
                            profit_loss,
                            trade_state.take_profit,
                        ...
                        profit_loss = (trade_state.entry_price - trade_state.trailing_stop) * qty
                        close_trade(
                            "Trailing Stop (profit locked)",
                            profit_loss,
                            trade_state.trailing_stop,
                        ...
                        profit_loss = (trade_state.entry_price - trade_state.stop_loss) * qty
                        close_trade(
                            "Stop Loss",
                            profit_loss,
                            trade_state.stop_loss,
