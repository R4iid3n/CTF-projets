...
# Based on the psychology of Yuichi Katagiri (Tomodachi Game)

"""
YUICHI PHILOSOPHY (simplified):

1. Acts only when he is several moves ahead.
2. Observes behavior to detect patterns.
3. Exploits greed and fear of others.
4. Turns manipulations against the manipulators.
5. Extreme patience: waits for perfect setup.
6. Strikes hard when opponent is locked in his own trap.
"""
...
logging.basicConfig(
    ...
    level=logging.INFO,
    handlers=[
        logging.FileHandler("yuichi_bot_v14_aggresive.log"),
...
class TrapType(Enum):
    BULL_TRAP = "bull_trap"           # Fake breakout up
    BEAR_TRAP = "bear_trap"           # Fake breakout down
    LIQUIDITY_GRAB = "liquidity_grab" # Stop hunt
    WHALE_ACCUMULATION = "whale_accumulation"
    WHALE_DISTRIBUTION = "whale_distribution"
    RETAIL_FOMO = "retail_fomo"       # Retail buying the top
    RETAIL_PANIC = "retail_panic"     # Retail selling the bottom
...
class YuichiConfig:
    ...
    max_trades_per_day = 15  # slightly higher

    # Position sizing base units
    base_positions = {
        "observing": 0,
        "trap_detected": 60,
        "setup_forming": 90,
    ...
    martingale_multipliers = [1.0, 1.5, 2.2, 3.5, 5.5, 8.0]
    current_martingale_level = 0
    max_martingale_level = 3

    # More aggressive
    min_trap_confidence = 60.0   # was 75
    min_setup_score = 6.0        # was 7
    min_time_between_trades = 10 # minutes (was 20)
    last_trade_time = None

    min_observation_candles = 30 # was 50
    ...
    # Battle stats
    battle_step = 0  # increments every trade
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    logger.error(f"Failed after {max_retries}: {e}")
                    return None
...
def detect_yuichi_trap(df_5m, df_15m, df_1h, order_book) -> tuple:
    ...
    Returns: (TrapType, confidence 0-100)
    """
    ...
    if price > recent_high_5m:
        ...
        if confidence >= 50:
            ...
            logger.warning(f"[TRAP] BULL TRAP detected | confidence={confidence:.0f}%")
    ...
    # BEAR TRAP
        ...
        if confidence >= 50:
            ...
            logger.warning(f"[TRAP] BEAR TRAP detected | confidence={confidence:.0f}%")
    ...
    if df_5m is not None and len(df_5m) >= 5:
        ...
        if drop < -0.01 and bounce > 0.005:
            ...
            if volume_spike > 1.5:
                ...
                logger.warning(f"[TRAP] LIQUIDITY GRAB detected | confidence={confidence:.0f}%")

    # RETAIL FOMO (strict version)
    if rsi_5m > 80:
        ...
        if volume_explosion > 2.0 and candle_range < 0.003:
            ...
            logger.warning(f"[TRAP] RETAIL FOMO detected | confidence={confidence:.0f}%")

    # RETAIL PANIC (strict version)
    elif rsi_5m < 20:
        volume_explosion = volume / avg_volume
        candle_range = (df_5m["high"].iloc[-1] - df_5m["low"].iloc[-1]) / df_5m["close"].iloc[-1]
        ...
        if volume_explosion > 2.0 and candle_range < 0.003:
            ...
            logger.warning(f"[TRAP] RETAIL PANIC detected | confidence={confidence:.0f}%")

    # FALLBACK: pure RSI traps (more aggressive)
    if trap is None:
        if rsi_5m > 75:
            trap = TrapType.RETAIL_FOMO
            confidence = max(confidence, 60)
            logger.warning(f"[TRAP-FALLBACK] FOMO (RSI>75) | confidence={confidence:.0f}%")
        elif rsi_5m < 25:
            trap = TrapType.RETAIL_PANIC
            confidence = max(confidence, 60)
            logger.warning(f"[TRAP-FALLBACK] PANIC (RSI<25) | confidence={confidence:.0f}%")
...
def calculate_yuichi_psychology(multi_tf_data, order_book) -> MarketPsychology:
    ...
    trap, trap_confidence = detect_yuichi_trap(df_5m, multi_tf_data.get("strategic"), df_1h, order_book)
...
def yuichi_entry_signal(multi_tf_data, psychology) -> tuple:
    if psychology.game_state == GameState.UNCERTAIN:
        return None, 0.0, "uncertain conditions"

    if psychology.game_state == GameState.OBSERVING:
        return None, 0.0, "observing phase"
    ...
    if not signal_type or score < config.min_setup_score:
        return None, score, "score too low"

    logger.info(
        f"[SIGNAL] type={signal_type.upper()} | score={score:.1f}/10 | "
        f"trap={trap.value if trap else 'none'} | confidence={trap_conf:.0f}%"
    )
...
def calculate_yuichi_position_size(psychology, score) -> float:
    ...
    if score >= 9.0:
        size *= 1.3
        logger.info("[SIZE] perfect setup: size boosted 30%")
...
def log_result(msg, log_box=None):
    try:
        with open("yuichi_true_trades.txt", "a", encoding="utf-8") as f:
...
def update_winnings_box(entry_var=None):
    logger.info(
        f"[WINNINGS] capital={config.capital:.2f} | "
        f"cumulative={config.cumulative_winnings:.2f} | "
        f"martingale_step={config.current_martingale_level} | "
        f"battle_step={config.battle_step}"
    )
...
def update_psychology_panel(box, psychology):
    if psychology is None:
        return
    logger.info(
        f"[PSY] sentiment={psychology.sentiment.value} | F&G={psychology.fear_greed_index:.0f} | "
        f"vol={psychology.volatility_regime} | game={psychology.game_state.value} | "
...
def close_trade(reason, exit_price, log_box=None, winnings_var=None):
    ...
    trade_data = {
        ...
        "martingale_step": config.current_martingale_level,
    ...
    log_msg = (
        f"\n{'='*80}\n"
        f"[BATTLE RESULT] {reason} [{result}]\n"
        f"{'-'*80}\n"
        f"BattleStep: {config.battle_step}\n"
        f"MartingaleStep: {config.current_martingale_level}\n"
        f"Type: {trade_state.type.upper()}\n"
        f"GameState: {trade_state.game_state.value if trade_state.game_state else 'unknown'}\n"
        f"Trap: {trade_state.trap_type.value if trade_state.trap_type else 'none'}\n"
        f"Score: {trade_state.score:.1f}/10\n"
        f"EntryPrice: {trade_state.entry_price:.2f}\n"
        f"StopLoss:  {trade_state.stop_loss:.2f}\n"
        f"TakeProfit:{trade_state.take_profit:.2f}\n"
        f"ExitPrice: {exit_price:.2f}\n"
        f"PositionSize: {trade_state.position_size:.2f}\n"
        f"GrossPnL: {gross_pnl:.2f}\n"
        f"Fees: {fees:.2f}\n"
        f"NetPnL: {net_pnl:.2f} ({roi_pct:+.2f}%)\n"
...
def execute_yuichi_strategy(log_box=None, ax=None, canvas=None, winnings_var=None, stats_box=None, psych_box=None):
    logger.info("=" * 70)
    logger.info("[INIT] Yuichi True Method (CLI)")
    logger.info("Patience. Observation. Precision. Execution.")
    logger.info("=" * 70)
    ...
    while config.running:
        try:
            if (config.trades_executed_today >= config.max_trades_per_day or
                ...
                log_result("Daily limits reached, pausing.", log_box)
                time.sleep(60)
                continue
            ...
            if not trade_state.active:
                ...
                if not signal_type:
                    logger.info(f"[SKIP] no entry: {reason} (score={score:.1f})")
                    time.sleep(5)
                    continue
                ...
                if expected_profit_pct < config.min_profit_threshold:
                    logger.info(
                        f"[SKIP] expected profit {expected_profit_pct:.2%} < "
                        f"threshold {config.min_profit_threshold:.2%}"
                    )
                    time.sleep(5)
                    continue
                ...
                if position_size == 0:
                    logger.info("[SKIP] position size is 0")
                    time.sleep(5)
                    continue
                ...
                log_msg = (
                    f"\n{'='*80}\n"
                    f"[BATTLE START] Yuichi enters {signal_type.upper()}\n"
                    f"{'-'*80}\n"
                    f"BattleStep: {config.battle_step}\n"
                    f"MartingaleStep: {config.current_martingale_level}\n"
                    f"GameState: {psychology.game_state.value.upper()}\n"
                    f"Trap: {psychology.trap_type.value if psychology.trap_type else 'none'}\n"
                    f"Confidence: {psychology.trap_confidence:.0f}%\n"
                    f"Score: {score:.1f}/10\n"
                    f"EntryPrice: {current_price:.2f}\n"
                    f"StopLoss:  {sl:.2f}\n"
                    f"TakeProfit:{tp:.2f}\n"
                    f"PositionSize: {position_size:.2f}\n"
                    f"CumulativeWinnings: {config.cumulative_winnings:.2f}\n"
                    f"{'='*80}"
                )
                log_result(log_msg, log_box)
...
if __name__ == "__main__":
    logger.info("[BOOT] Initializing Yuichi True Method (CLI)")
