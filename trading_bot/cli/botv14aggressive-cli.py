...
#
# Save as: botv14aggressive-cli.py
...
logging.basicConfig(
    ...
    level=logging.INFO,
    handlers=[
        logging.FileHandler("yuichi_bot_v14_aggressive.log"),
...
class MarketPsychology:
    ...
    retail_positioning: float          # RSI proxy
    smart_money_flow: float            # order_flow_delta
...
class YuichiConfig:
    ...
    martingale_multipliers = [1.0, 1.7, 3.0, 5.0, 8.0]
    ...
    def fetch_with_retry(self, func, *args, max_retries=3, **kwargs):
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(f"[EXCHANGE] Rate limit - waiting {wait_time:.1f}s")
                time.sleep(wait_time)
            except Exception as e:
                logger.error(f"[EXCHANGE] Error (attempt {attempt+1}/{max_retries}): {e}")
...
def yuichi_entry_signal(multi_tf_data, psychology) -> tuple:
    ...
    if psychology.game_state == GameState.OBSERVING:
        return None, 0.0, "Observation phase"
...
# ---------------------------------------------------------------------
# STATUS EXPORT (for Windows dashboard)
# ---------------------------------------------------------------------

def write_status(extra: dict | None = None):
    """Write current bot state to a JSON file status/yuichi_v14_aggressive_cli.json"""
    try:
        data = {
...
def log_result(msg, log_box=None):
    try:
        with open("yuichi_true_trades_v14_aggressive.txt", "a", encoding="utf-8") as f:
...
def update_winnings_box(entry_var=None):
    logger.info(f"[WINNINGS] Cumulative winnings: ${config.cumulative_winnings:.2f}")
...
def execute_yuichi_strategy(
    ...
    logger.info("=" * 70)
    logger.info("[INIT] Yuichi True Method (CLI AGGRESSIVE)")
    ...
    while config.running:
        try:
            ...
            if (config.trades_executed_today >= config.max_trades_per_day or
                ...
                log_result("[RISK] Daily limits reached, pausing.", log_box)
                write_status()
                time.sleep(60)
                continue
            ...
            # Active position
                ...
                if trade_state.type == "buy" and current_price >= trade_state.take_profit:
                    ...
                    # still in trade, just update status
                    write_status()
...
if __name__ == "__main__":
    logger.info("[BOOT] Initializing Yuichi True Method v14 AGGRESSIVE (CLI)")
    logger.info(f"[BOOT] Starting capital: {config.capital}")
    logger.info("[BOOT] Status JSON: status/yuichi_v14_aggressive_cli.json")
