...
class YuichiConfig:
    ...
    # FIXED: Lowered from 3 to 2 for more trade opportunities while maintaining quality
    ...
    # FIXED: Hedge activation threshold increased from 0.003 (0.3%) to 0.01 (1%)
...
class PerformanceTracker:
    def __init__(self):
        ...
        # NEW: Track confirmation effectiveness
        self.confirmation_win_rates: Dict[str, Dict] = {}
...
def analyze_whale_activity(order_book_data):
    """
    FIXED: More robust whale detection with higher depth requirement and capped ratios
    """
    ...
    # FIXED: Increased from 5.0 to 10.0 for more substantial depth requirement
    min_depth = 10.0
    ...
    # FIXED: Reduced max_ratio from 10.0 to 5.0 for more realistic detection
    max_ratio = 5.0
...
# ---------------------------------------------------------------------
# ENTRY LOGIC (v13 balanced BUY/SELL)
# ---------------------------------------------------------------------
...
def yuichi_entry_signal(multi_tf_data, psychology):
    ...
    # 1) Psychologie globale (juste un bias, pas une obligation)
    ...
    # 2) Traps / manipulation (contrarian)
    ...
    # 3) RSI extrêmes (symétrique long/short)
    ...
    # 5) EMA trend local (trend-follow)
    if ema9 > ema21 and price > ema9:
        confirmations.append("bullish_ema_trend")
        if signal_type == "sell":
            # battle between trend and contrarian; keep SELL only if trap present
    ...
    if trend_up_oversight is not None:
        if trend_up_oversight and signal_type == "buy":
            confirmations.append("uptrend_confirmed")
        elif (not trend_up_oversight) and signal_type == "sell":
            confirmations.append("downtrend_confirmed")
        else:
            # if going against 1h trend, mark as countertrend but do NOT block
            confirmations.append("countertrend_play")

    # 8) Dernier recours pour l'équilibre (si toujours aucune direction claire)
    if signal_type is None:
    ...
    # 9) Validation finale (FIXED: lowered from 3 to 2)
...
# ---------------------------------------------------------------------
# SL/TP SHRINK OVER TIME - FIXED: Gentler shrinkage
# ---------------------------------------------------------------------
...
def tighten_sl_tp_over_time(trade_duration_minutes):
    ...
    # FIXED: Changed from 0.7 (70% shrinkage) to 0.4 (40% shrinkage)
    # Now shrinks to 60% instead of 30% of original distance
    shrink = 1.0 - 0.4 * progress
    # FIXED: Never below 60% instead of 30%
    shrink = max(0.6, shrink)
    ...
    while config.running:
        try:
            ...
            # ------------------ ACTIVE POSITION ------------------
                ...
                # FIXED: Hedge bet activation with new thresholds
                if not trade_state.hedge_active:
                    ...
                    # FIXED: Changed from 0.003 (0.3%) to 0.01 (1%)
                    if (
                        ...
                        # FIXED: Changed from 0.5 (50%) to 0.3 (30%)
                        ...
                        log_result(
                            ...
                            f"(hedge notional: ${hedge_size:.2f} = {config.hedge_size_ratio*100:.0f}% of position)",
                            log_box
                        )
                ...
                if trade_state.hedge_active and trade_state.hedge_entry_price:
                    ...
                    hedge_qty = (
                        trade_state.hedge_size / trade_state.hedge_entry_price
                        if trade_state.hedge_entry_price > 0 else 0.0
                    )
                    hedge_pnl = price_diff * hedge_qty
                ...
                if trade_duration > config.max_hold_time_minutes:
                    ...
                    main_qty = (
                        trade_state.main_size / trade_state.entry_price
                ...
                # TP / SL / trailing for main position
                main_qty = (
                    trade_state.main_size / trade_state.entry_price
                    if trade_state.entry_price and trade_state.main_size > 0
                    else 0.0
...
def create_gui():
    root = tk.Tk()
    root.title("Trading Bot v13: Special Method - FIXED")
    ...
    def stop():
        ...
        log_result("Bot stopped by user.", log_box)
...
if __name__ == "__main__":
    logger.info("Initializing Trading System v13 FIXED...")
    logger.info(f"Capital: ${config.capital}")
    logger.info("Mode: Psychological warfare with OPTIMIZED parameters")
    logger.info("IMPROVEMENTS:")
    logger.info("  - Position sizes increased 6x (now $50-$1600)")
    logger.info("  - Hedge activation: 1% drawdown (was 0.3%)")
    logger.info("  - Hedge size: 30% (was 50%)")
    logger.info("  - SL shrinkage: 40% max (was 70%)")
    logger.info("  - Min confirmations: 2 (was 3)")
    logger.info("  - Battle multipliers: more aggressive")
    logger.info("  - Whale detection: stricter thresholds")
    create_gui()